...
document.getElementById('start').innerHTML = unsortedArr.join(", ");

function sortArray(){
  var sortedArray = bubbleSort(unsortedArr);

  function bubbleSort(arr){
    console.log(arr);
    // for the length of the array
    for (var i = 0; i < arr.length; i++){
      // for each pair of values in the array starting 0
      for (var j = 0; j < arr.length - 1; j++){
        // arr[j] // first number
        // arr[j + 1] // second number
        // if first > second
        if(arr[j] > arr[j + 1]){
          // temp = second
          var temp = arr[j + 1];
