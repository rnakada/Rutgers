...
SELECT title, firstName, lastName, authorId, authors.id
FROM books
...
/* SELECT product.name, product.link, category.name, category.link
FROM 
product LEFT JOIN category 
ON product.id = category.product_id
WHERE search_keyword = "pride and prejudice"; */
